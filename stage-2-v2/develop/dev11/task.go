...
func main() {
	configPath := flag.String("config", "config/config.toml", "путь доя файла с конфигурацией сервиса")
	flag.Parse()
	...
	// инифиализация адаптера (связь с данным сервисом)
	api := adapter.New(cfg, srv)
	// запуск адаптера
	errChannel := api.Start()
	// остановка адаптера
	defer func() {
